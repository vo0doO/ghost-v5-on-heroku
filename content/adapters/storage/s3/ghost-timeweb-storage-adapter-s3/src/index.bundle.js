...
class Store extends BaseStore {
  ...
  save(image, targetDir) {
    ...
    return new Promise((resolve, reject) => {
      Promise.all([this.getUniqueFileName(image, directory), readFileAsync(image.path)]).then(([fileName, file]) => {
        let config = {
          ACL: this.acl,
          Body: file,
          Bucket: this.bucket,
  ...
  serve() {
    ...
    }).on('httpHeaders', (statusCode, headers, response) => res.set(headers)).createReadStream().on('error', err => {
      res.status(404);
      next(err);
    }).pipe(res);
  }
